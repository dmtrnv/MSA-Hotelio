...
# Проверка соединения
echo "🧪 Проверка подключения к БД микросервиса..."
timeout 2 bash -c "</dev/tcp/${MS_DB_HOST}/${MS_DB_PORT}" \
  || { echo "❌ Не удалось подключиться к ${MS_DB_HOST}:${MS_DB_PORT}"; exit 1; }

# Загрузка фикстур
echo "🧪 Загрузка фикстур микросервиса..."
PGPASSWORD="${MS_DB_PASSWORD}" psql -h "${MS_DB_HOST}" -p "${MS_DB_PORT}" -U "${MS_DB_USER}" "${MS_DB_NAME}" < ms-init-fixtures.sql
